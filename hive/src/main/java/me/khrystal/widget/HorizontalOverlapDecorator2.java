...
public class HorizontalOverlapDecorator2 extends RecyclerView.ItemDecoration {
    private int mSpaceHeight;
    private int mSpaceWidth;
    private final int mRowSize;

    public HorizontalOverlapDecorator2(int rowSize, float mSpaceWidth, float mSpaceHeight) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        outRect.set((int) Math.ceil(offset_left ),
                (int) Math.ceil(offset_up+ mSpaceWidth / 2),
                (int) Math.ceil(offset_right ),
                (int) Math.ceil(offset_down)+ mSpaceWidth / 2);
